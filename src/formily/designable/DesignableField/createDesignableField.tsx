...
import { DesignableObject } from '@formily/designable-antd/esm/components/DesignableObject';
import { FormItemSwitcher } from '@formily/designable-antd/esm/components/FormItemSwitcher';
import * as defaultSchemas from '@formily/designable-antd/esm/schemas';
import { includesComponent } from '@formily/designable-antd/esm/shared';
import { DataSourceSetter, ReactionsSetter, ValidatorSetter } from '@formily/designable-setters';
import {
  ArrayField,
  Field,
  ObjectField,
...
export const createDesignableField = (options: IDesignableFieldFactoryProps) => {
  ...
  const getFieldPropsSchema = (node: TreeNode): ISchema => {
          ...
          propsTab: {
            ...
            properties: {
              propsPane: {
                ...
                'x-component-props': {
                  tab: GlobalRegistry.getDesignerMessage(`settings.${key}.tab_property`),
                },
              ...
              stylePane: {
                ...
                'x-component': 'FormTab.TabPane',
                'x-component-props': {
                  tab: GlobalRegistry.getDesignerMessage(`settings.${key}.tab_style`),
                },
                properties: {
    ...
    const base = {
      ...
      properties: {
        ...
        //   'x-index': 4,
        // },
        'x-component-props': componentSchema && TabSchema('x-component-props', componentSchema),
        // 'x-decorator-props': decoratorSchema && TabSchema('x-decorator-props', decoratorSchema),
      },
    };
  ...
  const calculateChildrenRestricts = (target: TreeNode, source: TreeNode[]) => {
    ...
    const restrictChildrenComponents = realOptions.restrictChildrenComponents?.[targetComponent];
    if (restrictChildrenComponents?.length) {
      if (source.every((node) => includesComponent(node, restrictChildrenComponents, target))) {
        return true;
      }
      return false;
    }
  ...
  const calculateSiblingsRestricts = (target: TreeNode, source: TreeNode[]) => {
    const targetComponent = target.props['x-component'];
    const restrictSiblingComponents = realOptions.restrictSiblingComponents?.[targetComponent];
    if (restrictSiblingComponents?.length) {
      if (source.every((node) => includesComponent(node, restrictSiblingComponents, target))) {
        return true;
      }
      return false;
    }
  ...
  GlobalRegistry.registerDesignerProps({
    [realOptions.registryName]: (node) => {
      ...
      const message = GlobalRegistry.getDesignerMessage(`components.${componentName}`);
      ...
      return {
        ...
        selfRenderChildren:
          isArrayNode || includesComponent(node, realOptions.selfRenderChildrenComponents),
        inlineLayout: includesComponent(node, realOptions.inlineLayoutComponents),
        inlineChildrenLayout: includesComponent(node, realOptions.inlineChildrenLayoutComponents),
  ...
  const DesignableField: React.FC<ISchema> = observer((props) => {
    ...
    const getFieldProps = () => {
      ...
      if (fieldProps.decorator?.[0]) {
        ...
        FormPath.setIn(fieldProps.decorator[1], designer.props.nodeIdAttrName, node.id);
      } else if (fieldProps.component?.[0]) {
        fieldProps.component[1] = fieldProps.component[1] || {};
        FormPath.setIn(fieldProps.component[1], designer.props.nodeIdAttrName, node.id);
      }
